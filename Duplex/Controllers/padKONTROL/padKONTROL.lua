...
function padKONTROL:sysex_callback(message)
  ...
  if message[#header+1] == 0x45 then
     ...
     if (n >= 0x40) and (n <= 0x4F) then
        ...
        for k, v in ipairs(self.control_map:get_midi_params(str)) do
           local msg = Message()
           msg.value = value
           msg.is_note_off = false
           msg.context = DEVICE_MESSAGE.MIDI_NOTE
           self:_send_message(msg, v)
        end
  ...
  elseif message[#header+1] == 0x4B then
     ...
     for k, v in ipairs(self.control_map:get_midi_params("XYPAD")) do
        local msg = Message()
        msg.value = {x, y}
        self:_send_message(msg, v)
     end
...
function padKONTROL:midi_callback(message)
end

--[[
function padKONTROL:output_value(pt, elm, ceiling)
   if type(pt.val) == "table" then
      ...
      return Device.output_value(self, pt, elm, ceiling)
   end
end
]]
